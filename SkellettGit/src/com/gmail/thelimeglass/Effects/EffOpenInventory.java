	...
	protected void execute(Event e) {
		...
		for (Player p : players.getAll(e)) {
			if (p.getOpenInventory() != null) {
				//Constructor<?> ConstructorActionbar = ReflectionUtil.getNMSClass("PacketPlayOutOpenWindow").getDeclaredConstructor(getHandle.activeContainer.windowId, "minecraft:chest", new ChatMessage(title), p.getOpenInventory().getTopInventory().getSize());
				p.closeInventory();
			}
			p.openInventory(inv);
		}
