		...
		if (isOpen()) {
			try {
				...
				if (Skellett.spData.get("Heartbeat") == null) {
					Skellett.spData.set("Heartbeat", 50);
					try {
				...
				ArrayList<Object> data = new ArrayList<Object>(Arrays.asList(Skellett.spData.getBoolean("Events", false), Skellett.spData.getInt("EventPort", 7331), Bukkit.getPort(), Bukkit.getServerName(), Skellett.spData.getInt("Heartbeat", 50) * 60, Bukkit.getMotd(), Bukkit.getMaxPlayers()));
